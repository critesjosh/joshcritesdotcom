...
This is a lightweight Jekyll site hosted at [joshcrites.com](https://joshcrites.com). The homepage remains a hand-authored static page; Jekyll is used only to turn Markdown essays into consistent article pages. Cloudflare Pages runs `bundle exec jekyll build` and publishes the generated `_site/` directory.
...
Install the Ruby dependencies with `bundle install`, then preview with `bundle exec jekyll serve` and open `http://localhost:4000`. Cloudflare Pages creates a preview deployment for pull requests and publishes `main` to the production site.
